...
class Cluster(SageObject):
    ...
    # TODO set _frobenius from from_roots, if phi and rho are none try and guess them from type
    @classmethod
    ...
    def dual_graph(self, with_gal=False, check=True):
        ...
        if (not self.is_principal()) and len(self.children()) == 2 and self.is_even():
            if self.children()[0].is_even():  # both even
                ...
                if (self.children()[0].size() == 2 and self.children()[1].is_principal()) or (self.children()[1].size() == 2 and self.children()[0].is_principal()):
                    t = next(S for S in self.children() if S.is_twin())
                    s = next(S for S in self.children() if not S.is_twin())
                    if s.is_ubereven():
                        Gamma_sp = (s,+1)
                        Gamma_sm = (s,-1)
                    else:
                        Gamma_sp, Gamma_sm = s, s
                    L = [Gamma_sp] +[(Gamma_sp, t, Gamma_sm, i) for i in range(2*(s.relative_depth() + t.relative_depth()) - 1)] + [Gamma_sm]
                    G.add_edges([(L[i], L[i+1]) for i in range(len(L)-1)])
                elif self.children()[0].is_principal() and self.children()[1].is_principal():
                    s1 = self.children()[0]
                    s2 = self.children()[1]
                    if s1.is_ubereven():
                        Gamma_s1p = (s1,+1)
                        ...
                        Gamma_s1p, Gamma_s1m = s1, s1
                    if s2.is_ubereven():
                        Gamma_s2p = (s2,+1)
                        Gamma_s2m = (s2,-1)
                    else:
                        Gamma_s2p, Gamma_s2m = s2, s2
                    Lp = [Gamma_s1p] +[(Gamma_s1p, Gamma_s2p, +1, i) for i in range(s1.relative_depth() + s2.relative_depth() - 1)] + [Gamma_s2p]
                    G.add_edges([(Lp[i], Lp[i+1]) for i in range(len(Lp)-1)])
                    Lp = [Gamma_s1m] +[(Gamma_s1m, Gamma_s2m, -1, i) for i in range(s1.relative_depth() + s2.relative_depth() - 1)] + [Gamma_s2m]
                    G.add_edges([(Lp[i], Lp[i+1]) for i in range(len(Lp)-1)])
    ...
    def integral_differential_lattice(self):
        ...
        for i in range(g):
            ...
            for t in self.all_descendants():
                if not t.is_proper():
                    continue
                # TODO this is inefficient, each step can be updated from the last
